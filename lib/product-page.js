...
function renderProductPage(p, related){
  ...
  const descr=`${esc(ttl)}. ${esc(p.res?p.res+'. ':'')}${esc((p.desc||'').slice(0,140))} Поставка, монтаж, гарантия. Усть-Каменогорск.`;
  ...
  let descBlock='';
  if(p.desc){
    const parts=String(p.desc).split(/[;\n•·]|,(?![^()]*\))/).map(s=>s.trim().replace(/\s+/g,' ')).filter(s=>s.length>=2&&s.length<=140);
