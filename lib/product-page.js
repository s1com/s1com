...
function renderProductPage(p, related){
  const title=`${esc(p.brand?p.brand+' ':'')}${esc(p.model)} — купить в Усть-Каменогорске | Сервис.com`;
  const descr=`${esc(p.brand?p.brand+' ':'')}${esc(p.model)}. ${esc(p.res?p.res+'. ':'')}${esc((p.desc||'').slice(0,140))} Поставка, монтаж, гарантия. Усть-Каменогорск.`;
  ...
  const ld={"@context":"https://schema.org/","@type":"Product","name":`${p.brand?p.brand+' ':''}${p.model}`,
    "description":(p.desc||'').slice(0,300),"brand":{"@type":"Brand","name":p.brand||'Сервис.com'},
    "category":p.group, "image":img};
  if(p.price) ld.offers={"@type":"Offer","price":p.price,"priceCurrency":"KZT","availability":p.inStock?"https://schema.org/InStock":"https://schema.org/PreOrder","url":url,"seller":{"@type":"Organization","name":"Сервис.com"}};
  const ldBread={"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[
    ...
    {"@type":"ListItem","position":3,"name":`${p.brand?p.brand+' ':''}${p.model}`,"item":url}]};

  const specs=[['Бренд',p.brand],['Модель',p.model],['Направление',p.group],['Тип',p.type],['Разрешение',p.mp],['Подключение',(p.conn||[]).join(', ')]]
  ...
  const priceBlock = p.price
    ? `<div class="pd-price">${p.promo&&p.oldprice?`<span class="old">${fmt(p.oldprice)}</span> `:''}${fmt(p.price)} <small>РРЦ</small></div>
       <div class="pd-stock">${p.inStock?'✓ В наличии':'Под заказ'} · опт — при заказе</div>`
...
<div class="pd-wrap">
  ...
  <div class="pd">
    <div class="pd-img">${imgTag}</div>
    <div class="pd-info">
      <div class="pd-brand">${esc(p.brand||p.cat||'')}</div>
      <h1>${esc(p.brand?p.brand+' ':'')}${esc(p.model)}</h1>
      ${priceBlock}
  ...
  ${p.desc?`<div class="pd-desc"><h2>Описание</h2><p>${esc(p.desc)}</p></div>`:''}
